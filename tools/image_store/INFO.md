...
## Input parameters
- image: base64-encoded image data, without the `data:…;base64,` prefix (required)
- mime: MIME type of the image — one of `image/jpeg`, `image/png`, `image/gif`, `image/webp` (optional, default: `image/jpeg`)
- filename: original filename hint for debugging — not used in the stored path (optional)
## Output
